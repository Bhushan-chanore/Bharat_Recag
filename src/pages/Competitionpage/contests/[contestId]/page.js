...
const Page = () => {
  ...
  return (
    <div className="w-full flex flex-col gap-5 h-[100%]  mx-auto sm:p-[3rem] p-[0.5rem] bg-white">
      <div>
        <Link to="/competition" className="font-semibold text-gray-400">
         ⯇ Back to Contests
        </Link>
      </div>
      ...
      <div className="flex gap-5 max-md:flex-col max-md:items-center">
        <div className="flex-grow">
          ...
          <div className="bg-light-3 rounded-r-full pl-4 pr-10 py-4 mb-5 w-fit">
            <h1 className="text-2xl">Weekly Contest {contestId.slice(15)}</h1>
          </div>
        ...
        <div className="mt-10 flex items-start gap-5 max-md:flex-col">
          <div className="w-full border border-light-3 rounded-lg hover:shadow-lg transition-all">
            ...
            {problems.map((prob, index) => (
              <Link key={index} to={`/competition/${contestId}/${prob.title.toLowerCase().split(' ').join('-')}`}
          ...
          <div className="w-full border border-light-3 rounded-lg hover:shadow-lg transition-all">
            ...
            {rankings.map((ranking, index) => (
              <div key={index} className="flex bg-light-1 px-4 py-2.5 hover:bg-light-2 border-b border-light-3">
                ...
                <Link to={`/profile/${ranking.username}`} className="text-sm w-[40%] hover:text-blue-500 text-center">
                  {ranking.username}
                </Link>
